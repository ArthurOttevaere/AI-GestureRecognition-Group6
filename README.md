...
This project implements a gesture recognition pipeline on two 3D hand-gesture datasets (Domain 1 and Domain 4). It evaluates four classification methods across two cross-validation settings:

- **Edit Distance** (k-NN on k-means cluster sequences)
- **DTW** – Dynamic Time Warping (k-NN on standardised 3D sequences)
- **Random Forest** (on 42 hand-crafted kinematic features)
- **LSTM** (deep learning on padded sequences)
...
Statistical significance is assessed with Wilcoxon signed-rank tests and Bonferroni correction.
...
├── config.py             # Shared constants: data paths, DATA_DIR
...
├── preprocessing.py      # Standardisation, PCA, k-means cluster encoding
├── distance_metrics.py   # DTW and Edit Distance (Numba JIT-compiled)
├── classifiers.py        # Generic k-NN predictor
├── features.py           # 42-feature extraction, build_feature_dataset
...
├── results.py            # CSV export, Wilcoxon tests, p-value tables
...
| `results_domain{1,4}_{ui,ud}_{method}.csv` | Per-fold accuracies |
| `p_values_domain{1,4}.csv` | Wilcoxon p-value matrices |
