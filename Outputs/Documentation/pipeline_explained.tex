...
\cite{quinlan1986id3, breiman1984cart}. GridSearchCV with inner \textbf{5-fold} CV
(cf.\ Sec.~\ref{sec:innercv}).

\textbf{Regularisation against Overfitting:} In early iterations, the tree was allowed to grow indefinitely (\verb|max_depth=None|), causing severe memorisation of the training set (Train Acc = 1.000). To combat this high variance, the grid is strictly regularised:
\begin{itemize}[itemsep=0.1em,topsep=0.2em]
    \item \verb|max_depth| is restricted to $\{5, 10, 15\}$.
    \item \verb|min_samples_split| is evaluated over $\{5, 10, 20\}$.
    \item \verb|min_samples_leaf| is enforced in $\{2, 5, 10\}$ to prevent the creation of highly specific rules for single noisy samples.
\end{itemize}
...
GridSearchCV over \verb|n_estimators|, \verb|max_depth|, \verb|min_samples_split| and \verb|min_samples_leaf| with \textbf{inner 5-fold} CV
(Sec.~\ref{sec:innercv}).

\textbf{Regularisation against Overfitting:} Similarly to the Decision Tree, the Random Forest's base estimators are prevented from fully developing to control variance. The search space forces pruned trees (\verb|max_depth| up to 15, \verb|min_samples_leaf| $\in \{2, 5\}$). This ensures the Random Forest learns generalizable spatial patterns rather than memorising the dataset, achieving an excellent bias-variance tradeoff.
...
Le prof a fait remarquer qu'une accuracy d'entraînement de 100\,\% est
souvent suspecte et trahit un problème de haute variance (overfitting).
Le code intègre un diagnostic complet pour mesurer et contrôler ce biais.

\subsection{Gap train vs.\ test par fold}

Pour chaque \emph{classifieur paramétrique} (DT, RF, SVM, LR), les
fonctions \texttt{crossval\_\{ui,ud\}\_*} retournent la liste des
accuracies d'entraînement et de test. Le rapport produit \texttt{overfitting\_gap.csv} avec :
\[
\text{Gap} \;=\; \overline{\text{train\_acc}} \;-\; \overline{\text{test\_acc}}.
\]
Grâce aux hyperparamètres très restrictifs imposés dans \texttt{GridSearchCV}
(\texttt{max\_depth}, \texttt{min\_samples\_leaf}), l'overfitting a été drastiquement
réduit : la variance des Random Forests et SVM en User-Dependent plafonne
désormais sous la barre des 5\,\% d'écart. En User-Independent, le gap
(10 à 15\,\%) traduit l'inconstance naturelle du geste humain, que le modèle ne peut pas
mémoriser abusivement sans pénaliser son score de test.
...
\item \textbf{Écart train$-$val large et persistant} = high variance =
      overfitting. Remède apporté ici : élagage des arbres (\texttt{max\_depth}),
      mise à l'échelle rigoureuse (\texttt{Pipeline}) pour SVM/LR.
\item \textbf{Train et val tous deux bas} = high bias = underfitting.
      Symptôme rare ici grâce à la richesse des 50 descripteurs manuels.
\item \textbf{Train et val proches et élevés, plateau atteint} =
      modèle bien calibré, ce qui correspond à nos courbes finales post-régularisation.
\end{itemize}
